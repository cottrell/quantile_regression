...
class Dense:
    ...
    def __init__(self, activation=None, kernel_initializer=None, kernel_constraint=None, dtype=np.float64):
        self._kernel = None
        self._bias = None
...
class QuantileNetworkNoX:
    def __init__(self, *, input_dim, weight_dim, dims):
        ...
        self._my_layers = make_layers(
            input_dim=input_dim, weight_dim=weight_dim, dims=dims, activation="tanh", kernel_constraint="nonneg"
        )
...
def check_tau(tau):
    ...
    assert (
        tau.shape[1] == 1
    ), f'I do not think it ever makes sense to have 2nd dimension of tau be different from one. Got {tau.shape}'
...
def sanity_plot_nox(steps=1000):
    ...
    y = l["y"]
    weight_dim = 20
    model = QuantileNetworkNoX(input_dim=1, weight_dim=weight_dim, dims=[16, 16, 1])  # tau.shape[1] if tau is still 2d
